...
function E($msg, $code=0) {
    throw new Any\Exception($msg, $code);
}
...
function I($name,$default='',$filter=null,$datas=null) {
    ...
    is_array($data) && array_walk_recursive($data,'any_filter');
    return $data;
}
...
function import($class, $baseUrl = '', $ext=EXT) {
    ...
    if (empty($baseUrl)) {
        ...
        }elseif (in_array($class_strut[0],array('Any','Org','Behavior','Com','Vendor')) || is_dir(LIB_PATH.$class_strut[0])) {
            // 系统类库包和第三方类库包
            $baseUrl = LIB_PATH;
        }else { // 加载其他模块的类库
            $baseUrl = APP_PATH;
...
function D($name='',$layer='') {
    if(empty($name)) return new Any\Model;
    static $_model  =   array();
    ...
    if(class_exists($class)) {
        ...
        $model      =   class_exists($class)? new $class($name) : new Any\Model($name);
    }else {
        Any\Log::record('D方法实例化没找到模型类'.$class,Any\Log::NOTICE);
        $model      =   new Any\Model(basename($name));
    }
...
function B($name, $tag='',&$params=NULL) {
    ...
    return \Any\Hook::exec($name,$tag,$params);
}
 ...
 * 自定义异常处理
 * @param string $msg 异常消息
 * @param string $type 异常类型 默认为Any\Exception
 * @param integer $code 异常代码 默认为0
 * @return void
 */
function throw_exception($msg, $type='Any\\Exception', $code=0) {
    Any\Log::record('建议使用E方法替代throw_exception',Any\Log::NOTICE);
    if (class_exists($type, false))
        throw new $type($msg, $code);
    else
        Any\Any::halt($msg);        // 异常类型不存在则输出错误信息字串
}
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if(isset($route)) {
            ...
            $module =   (defined('MAIN_MODULE') && MAIN_MODULE==$module )? '' : $module;
...
function S($name,$value='',$options=null) {
    ...
    if(is_array($options)){
        ...
        $cache      =   Any\Cache::getInstance($type,$options);
    }elseif(is_array($name)) { // 缓存初始化
        $type       =   isset($name['type'])?$name['type']:'';
        $cache      =   Any\Cache::getInstance($type,$name);
        return $cache;
    }elseif(empty($cache)) { // 自动初始化
        $cache      =   Any\Cache::getInstance();
    }
    if(''=== $value){ // 获取缓存
...
function F($name, $value='', $path=DATA_PATH) {
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            Any\Storage::put($filename,serialize($value),'F');
            // 缓存数据
            $_cache[$name]  =   $value;
            return null;
        }
    ...
    if (isset($_cache[$name]))
        return $_cache[$name];
    if (Any\Storage::has($filename,'F')){
        $value      =   unserialize(Any\Storage::read($filename,'F'));
        $_cache[$name]  =   $value;
    } else {
        $value          =   false;
    }
 ...
 * @return string
 */
function xml_encode($data, $root='any', $item='item', $attr='', $id='id', $encoding='utf-8') {
    if(is_array($attr)){
        $_attr = array();
...
function cookie($name='', $value='', $option=null) {
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            ...
            // 设置cookie
            if(is_array($value)){
                $value  = 'any:'.json_encode(array_map('urlencode',$value));
            }
